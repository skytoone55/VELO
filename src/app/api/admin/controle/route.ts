...
export async function GET(request: NextRequest) {
  ...
  let query = adminClient
    ...
    .select(`
      ...
      client:clients!livraisons_client_id_fkey!inner(id, raison_sociale, contact_nom, contact_prenom, telephone, reference_retina, commercial_assigne, commercial_code, commercial:commercial_code(code, nom, parent_code), depot_logistique_id, depot_retrait_id, velo_valide, fnuci_ids, in_enemat),
  ...
  // Filtre commercial — via jointure inner pour eviter URL trop longue quand un master matche 1000+ clients
  if (commercialCodes.length > 0) {
    const expanded = await expandCommercialCodes(adminClient as any, getTenantId(), commercialCodes)
    if (expanded) {
      if (expanded.length === 1) query = query.eq('client.commercial_code', expanded[0])
      else query = query.in('client.commercial_code', expanded)
    }
  }
